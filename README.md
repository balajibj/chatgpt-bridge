...
Interactive mode and the real-browser E2E runner fingerprint the bundled extension and publish changed files into the stable `~/.local/share/chatgpt-bridge/extension` root before startup checks. Interactive mode uses the `ask` policy by default. After the operator accepts an update, Bridge opens a dedicated ChatGPT bootstrap tab, waits for the currently installed Protocol 5 runtime to connect from that exact tab, reloads the unpacked extension, and verifies that the same browser tab reconnects with the bundled manifest, content-runtime, and bundle identities. The interactive UI does not start if an accepted update fails; it never continues into a session that can only report an incompatible extension. Real E2E uses `if-needed` by default and performs the same verified update automatically before any scenario runs. Any changed bundle is reloaded automatically, including same-version development fixes. Before restarting the extension, the bootstrap ChatGPT tab is moved to a loopback reload trampoline served by Bridge. That ordinary web page survives service-worker teardown and returns the same browser tab to its exact ChatGPT URL after the new extension package is active, causing Chrome to inject the new content runtime without a manual page refresh. If no tab is connected, startup opens one through the system browser; a version-incompatible Protocol 5 runtime is still allowed to receive only the reload control command. Older-protocol clients cannot receive the reload command and must be updated manually through the installer.
...
Generated-file actions are selected by exact filename, by a stable block/action locator inside the source assistant turn, or by one unambiguous exact action label when ChatGPT exposes no filename metadata. `selectorHint` is never treated as identity because several file buttons can share the same CSS path, and duplicate generic labels remain fail-closed. Action readiness and download completion are separate wait domains: the exact usable action is resolved first, capture channels are armed second, and the download timer starts only after the action is activated. A proven action-readiness failure enters one explicit recovery action instead of leaving real E2E waiting until the workflow-wide deadline.
...
ChatGPT can occasionally accept a user turn and then render a localized transient error directly below that turn, such as `Что-то пошло не так. Попробуйте еще раз.` or `Something went wrong. Please try again.`. Bridge treats this as a typed ChatGPT-side failure only when the exact submitted user-turn key is still proved. It retries the same prompt inside the same lease and conversation with bounded exponential backoff (1s, 2s, and 4s by default), advancing the canonical response epoch for every attempt. Ambiguous or unconfirmed submissions are never retried. Configure the bounds with `CHATGPT_TRANSIENT_ERROR_MAX_RETRIES`, `CHATGPT_TRANSIENT_ERROR_RETRY_BASE_MS`, and `CHATGPT_TRANSIENT_ERROR_RETRY_MAX_MS`.
...
ChatGPT may expose an in-flight steer as a continuation user key while keeping the final assistant turn attached to the original prompt user key. The bridge records the new response epoch without assuming that both DOM keys must become identical. A split boundary is accepted only when the active request carries the proved steer key and the visible original key exactly matches the immediately previous response epoch; unrelated historical answers remain out of scope. Generation start alone is not treated as proof that steering is available: the server waits for semantic response progress or an explicit steer/send control, and the content runtime then waits for a real enabled Send button. It never substitutes an Enter keypress while the composer still exposes only Stop; if the steer window never opens, the inserted text is rolled back and the write is reported as proved not executed.
